...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg)
{
    ...
    char buf_display[64] = "Не назначено";
    ...
    if (*k != 0 && g.ActiveId != id) {
        ...
        strcpy_s(buf_display, ("Нажмите любую клавишу"));
    }
...
void ImGui::AddCircleImageFilled(ImTextureID user_texture_id, const ImVec2& centre, float radius, ImU32 col, int num_segments)
{
    ...
    ImGuiContext& g = *GImGui;
    GetForegroundDrawList()->PathClear();

    if ((col & IM_COL32_A_MASK) == 0 || num_segments <= 2)
        return;


    const bool push_texture_id = GetForegroundDrawList()->_TextureIdStack.empty() || user_texture_id != GetForegroundDrawList()->_TextureIdStack.back();
    if (push_texture_id)
        GetForegroundDrawList()->PushTextureID(user_texture_id);

    int vert_start_idx = GetForegroundDrawList()->VtxBuffer.Size;
    const float a_max = IM_PI * 2.0f * ((float)num_segments - 1.0f) / (float)num_segments;
   GetForegroundDrawList()->PathArcTo(centre, radius, 0.0f, a_max, num_segments - 1);
   GetForegroundDrawList()->PathFillConvex(col);
    int vert_end_idx = GetForegroundDrawList()->VtxBuffer.Size;

    ImGui::ShadeVertsLinearUV(GetForegroundDrawList(), vert_start_idx, vert_end_idx, ImVec2(centre.x - radius, centre.y - radius), ImVec2(centre.x + radius, centre.y + radius), ImVec2(0, 0), ImVec2(1, 1), true);

    if (push_texture_id)
        GetForegroundDrawList()->PopTextureID();
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float square_sz = 19.f;//GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos;
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x + 10.f, check_bb.Min.y + style.FramePadding.y - 3.f), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 18));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2({0.f, 8.f }));
    ...
    const bool hovered = ItemHoverable(frame_bb, id);
    const bool temp_input_allowed = 0;
    bool temp_input_is_active = temp_input_allowed && TempInputIsActive(id);
    if (!temp_input_is_active)
    {
    ...
    // Draw frame
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 5), ImVec2(frame_bb.Max.x, frame_bb.Max.y - 5), ImColor(18, 18, 18), 9);
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 5), ImVec2(grab_bb.Min.x, frame_bb.Max.y - 5), ImColor(18, 18, 18), 9);

    // Render grab
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 1, grab_bb.Min.y + 7.5f), 7, ImColor(255, 35, 35), 360);
    window->DrawList->AddShadowCircle(ImVec2(grab_bb.Min.x + 1, grab_bb.Min.y + 7.5f), 7, ImColor(255, 15, 15), 24.f, { 0.f, 0.f });

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);

    RenderText({ ImVec2(frame_bb.Max.x - 25, frame_bb.Min.y - 24).x - 5.f, ImVec2(frame_bb.Max.x + 5, frame_bb.Max.y - 5).y }, value_buf, value_buf_end);

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x + 10.f, frame_bb.Min.y + style.FramePadding.y - 3.f), label);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags, float alphaMax)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags, { 19.f, 19.f }))
        {
        ...
        float temp_position = alphaMax == -1.f ? 200.f : 250.f;


        ImGui::SetNextWindowSize({ 300.f, temp_position });
        //ImGui::PushStyleVar(ImGuiStyleVar_::ImGuiStyleVar_PopupRounding, 12.f);
        ImGui::PushStyleColor(ImGuiCol_PopupBg, ImColor(35, 35, 35).operator ImU32());
        ImGui::PushStyleColor(ImGuiCol_WindowShadow, ImVec4(0.f, 0.f, 0.f, 0.f));
        if (BeginPopup("picker", ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove))
        {
            ...
            if (alphaMax != -1.f) {
                SetCursorPosX(15.f);
                SetNextItemWidth(195.f);
                SliderFloat("Alpha", &col[3], 0.f, alphaMax);
            }
#define CALC(x, y, w, h) { x, y }, { x + w, y + h }
            ImGui::GetWindowDrawList()->AddRect(CALC(pos.x, pos.y + temp_position - 2.f, 300.f, 2.f), ImColor(255, 35, 35));
            EndPopup();
        }
       // ImGui::PopStyleVar();
    ...
    {
        window->DC.CursorPos = ImVec2(pos.x + style.ItemInnerSpacing.x + 28.f, pos.y + style.FramePadding.y - 3.f);
        //(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x + 10.f, check_bb.Min.y + style.FramePadding.y - 3.f), label);
